...
contains
  ...
  integer :: nxcen,nycen
  real(kind=wp) :: x,y,z,opac,taupole,taupole_dust, tauhomo,tauhomo_dust, N_gaspole,N_gashomo
  ...
  if (mpar%h_rank == 0) then
     ...
     if (par%density_zscale > 0.0_wp) then
        do k=1,grid%nz
        do j=1,grid%ny
        do i=1,grid%nx
     ...
     do j=1,grid%ny
     do i=1,grid%nx
        grid%rhokap(i,j,k) = grid%rhokap(i,j,k)/grid%Dfreq(i,j,k)*line%cross0
     enddo
     enddo
     enddo
     ...
     !--- We need to set tauhomoe = -999.0, taumax = -999.0 and N_gashomo = -999.0 in define.f90 (2020.08.27)
     if (par%taumax > 0.0_wp) then
        ...
        !--- the case when N(gas)_max is given.
        !--- N_gasmax is the radial or z-direction column density.
        opacity_sum = 0.0_wp
        do k=1, grid%nz
        ...
        if (par%xyz_symmetry) then
           if ((grid%nz/2)*2 == grid%nz) then
              opac_norm = par%N_gasmax/(opacity_sum*grid%dz/line%cross0)
           else
              opacity1  = grid%rhokap(nxcen,nycen,1)*grid%Dfreq(nxcen,nycen,1)
              opac_norm = par%N_gasmax/((opacity_sum - opacity1/2.0_wp)*grid%dz/line%cross0)
           endif
        else if (grid%zmax == -grid%zmin) then
           opac_norm = (2.0_wp*par%N_gasmax)/(opacity_sum*grid%dz/line%cross0)
        else
           opac_norm = par%N_gasmax/(opacity_sum*grid%dz/line%cross0)
        endif
        grid%rhokap(:,:,:) = grid%rhokap(:,:,:) * opac_norm
        if (par%DGR > 0.0_wp) grid%rhokapD(:,:,:) = grid%rhokapD(:,:,:) * opac_norm
     else if (par%N_gashomo > 0.0_wp) then
        !--- the case when N(coldens)_homo is given.
        !--- N_gashomo is the column density obtained for an homogenous medium that contains the same mass.
        opacity_sum = 0.0_wp
        nopac       = 0.0_wp
        do k=1, grid%nz
        do j=1, grid%ny
        ...
        enddo
        enddo
        dens               = opacity_sum/nopac/line%cross0
        opac_norm          = par%N_gashomo/(dens * opac_length)
        grid%rhokap(:,:,:) = grid%rhokap(:,:,:) * opac_norm
        if (par%DGR > 0.0_wp) grid%rhokapD(:,:,:) = grid%rhokapD(:,:,:) * opac_norm
     endif
     ...
     !--- calculate homogeneous N(coldens)
     opacity_sum = 0.0_wp
     nopac       = 0.0_wp
     do k=1, grid%nz
     do j=1, grid%ny
     ...
     enddo
     enddo
     N_gashomo  = opacity_sum/nopac/line%cross0 * opac_length

     !--- calculate N(coldens) along the pole direction
     opacity_sum = 0.0_wp
     do k=1, grid%nz
        opacity_sum = opacity_sum + grid%rhokap(nxcen,nycen,k)*grid%Dfreq(nxcen,nycen,k)
     enddo
     if (par%xyz_symmetry) then
        N_gaspole = opacity_sum*grid%dz/line%cross0
        opacity1 = grid%rhokap(nxcen,nycen,1)*grid%Dfreq(nxcen,nycen,1)
        if ((grid%nz/2)*2 /= grid%nz) N_gaspole = N_gaspole - opacity1*grid%dz/2.0_wp/line%cross0
     else if (grid%zmax == -grid%zmin) then
        N_gaspole = opacity_sum*grid%dz/2.0_wp/line%cross0
     else
        N_gaspole = opacity_sum*grid%dz/line%cross0
     endif
     ...
     if (par%N_gasmax  <= 0.0_wp) par%N_gasmax  = N_gaspole
     if (par%N_gashomo <= 0.0_wp) par%N_gashomo = N_gashomo
  endif
  call MPI_BCAST(par%N_gasmax,  1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_BCAST(par%N_gashomo, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  ...
  if (mpar%h_rank == 0) then
     if (len_trim(par%velo_file) > 0) then
        ...
        enddo
        enddo
     else if (trim(par%velocity_type) == 'parallel_velocity') then
        do k=1,grid%nz
        do j=1,grid%ny
        do i=1,grid%nx
           if (grid%rhokap(i,j,k) > 0.0_wp) then
        ...
        enddo
        enddo
     else if (trim(par%velocity_type) == 'rotating_solid_body') then
        !--- rotating solid body model in Garavito-Camargo et al. (2014, ApJ, 795, 120)
        !--- Vrot is the maximum rotation velocity at r = rmax in units of km/s.
        do k=1,grid%nz
        do j=1,grid%ny
        do i=1,grid%nx
           if (grid%rhokap(i,j,k) > 0.0_wp) then
        ...
        enddo
        enddo
     else if (trim(par%velocity_type) == 'rotating_galaxy_halo') then
        !--- rotating galaxy halo model in Kim et al.
        !--- Vrot is the maximum rotation velocity at r = rinner in units of km/s.
        do k=1,grid%nz
        do j=1,grid%ny
        do i=1,grid%nx
           if (grid%rhokap(i,j,k) > 0.0_wp) then
              ...
              if (rr < par%rinner) then
                 ...
                 grid%vfx(i,j,k) = -par%Vrot/vtherm * yy(j)/rr
                 grid%vfy(i,j,k) =  par%Vrot/vtherm * xx(i)/rr
              endif
           endif
        enddo
        enddo
  ...
  if (len_trim(par%emiss_file) > 0) then
     ...
     case ('density1')
        diffuse_emissivity_3D = .true.
        !--- when the emissivity is proportional to density.
        !--- grid%Pem : emissivity and probability distribution function for rejection method.
        call create_shared_mem(grid%Pem, [grid%nx,grid%ny,grid%nz])
        if (mpar%h_rank == 0) then
           do k=1,grid%nz
           do j=1,grid%ny
              grid%Pem(:,j,k) = grid%rhokap(:,j,k)
           enddo
           enddo
        endif
        call MPI_BARRIER(mpar%hostcomm, ierr)
     case ('density2')
        diffuse_emissivity_3D = .true.
        !--- when the emissivity is proportional to the square of density..
        !--- grid%Pem : emissivity and probability distribution function for rejection method.
        call create_shared_mem(grid%Pem, [grid%nx,grid%ny,grid%nz])
        if (mpar%h_rank == 0) then
           do k=1,grid%nz
           do j=1,grid%ny
              grid%Pem(:,j,k) = grid%rhokap(:,j,k)**2
           enddo
           enddo
        endif
        call MPI_BARRIER(mpar%hostcomm, ierr)
  ...
  !--- Calculate Probability and alias etc, if the diffuse emissivity is given on 3D grid system.
  if (diffuse_emissivity_3D) then
     ...
     case (0)
        ...
        call create_shared_mem(grid%alias, [size(grid%Pem1D)])
        if (mpar%h_rank == 0) then
           grid%Pem1D = grid%Pem1D/sum(grid%Pem1D)
           call random_alias_setup(grid%Pem1D, grid%alias)
        endif
     case (1)
        call create_shared_mem(grid%Pwgt,  [grid%nx, grid%ny, grid%nz])
        !--- bug-fixed (2023.10.20), originally, was size(grid%Pem1D), which is not yet defined.
        call create_shared_mem(grid%alias, [size(grid%Pwgt)])
        grid%Pem1D(1:size(grid%Pem))  => grid%Pem
        grid%Pwgt1D(1:size(grid%Pem)) => grid%Pwgt
     ...
     case default
        if (mpar%h_rank == 0) grid%Pem = grid%Pem/maxval(grid%Pem)
     endselect
     call MPI_BARRIER(mpar%hostcomm, ierr)
  endif
  ...
  if (mpar%h_rank == 0) then
     if (allocated(tmp_arr)) deallocate(tmp_arr)
  ...
  if (par%save_all_photons) then
     ...
     call create_shared_mem(allph%nscatt_gas,  [int(par%nphotons)])
  ...
  if (mpar%p_rank == 0) then
     ...
     write(6,'(a,es16.3)') 'N('//trim(line%ion_id)//')_pole     : ',N_gaspole
     write(6,'(a,es16.3)') 'N('//trim(line%ion_id)//')_homo     : ',N_gashomo
     write(6,'(a,es16.3)') 'tau_pole  ('//trim(line%ion_id)//') : ',taupole
     write(6,'(a,es16.3)') 'tau_homo  ('//trim(line%ion_id)//') : ',tauhomo
