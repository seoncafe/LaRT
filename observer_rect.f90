...
contains
  !---------------------------------------------------------------------------------
  !-----------------
  subroutine observer_create_outside()
  use mpi
  implicit none
  ...
  do i=1, par%nobs
     ...
     if (par%save_peeloff_3D) then
        ...
        if (par%save_dust_scattered) then
           call create_mem(observer(i)%scatt_dust,[par%nxfreq,par%nxim,par%nyim])
        endif
        if (par%use_stokes) then
  ...
  end subroutine observer_create_outside
  !-----------------
  subroutine observer_destroy_outside()
  use define
  use mpi
  implicit none
  integer :: ierr, i
  ...
  enddo
  end subroutine observer_destroy_outside
  !----------------------------------------
end module observer_rect
