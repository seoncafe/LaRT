...
idarr, rsUV,rsHI,rpeak,Vpeak,DelV,logtau,DGR,zarr, rlim,lambda1,lambda2 = np.loadtxt(result_table, skiprows=3, unpack=True)

#--- Open a file to record the scaling factors for spectrum and surface brightness.
fileout = open('muse_scaling_factors.txt','w')
...
for idx in np.arange(nf):
   ...
   if os.path.exists(fname+'.fits.gz'):
      plot_profiles(fname)
      npdf       = npdf + 1
      in_pdf_str = in_pdf_str+' '+fname+'_profile.pdf'

fileout.close

if npdf > 1:
   all_pdf = 'muse_profiles.pdf'
   os.system("gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=%s %s" % (all_pdf, in_pdf_str))
   #os.system("rm MUSE????_profile.pdf")
