...
for idx in arr:
   ...
   # For FeII UV3
   w_wavl,  = np.where((wavl > 2360.0) & (wavl < 2370.0))
   ...
   #--- Image for Q and U
   print(hdu3[0].data[:,:,w_wavl].shape)
   Iim = hdu3[0].data[:,:,w_wavl].sum(axis=2)
   Qim = hdu3[1].data[:,:,w_wavl].sum(axis=2)
   Uim = hdu3[2].data[:,:,w_wavl].sum(axis=2)
   ...
   #r      = hdu3[4].data['R']
   #pol    = hdu3[4].data['POL']
   pmax   = np.int32(np.int32(np.amax(polim)*100)/5.0+1)*5.0/100.0
   #pmax   = np.int32(np.nanmax(polim)*100.0)/100.0
   ...
   Imax  = np.amax(Iim[np.where(Iim > 0.0)])
   vmax  = np.log10(Imax)
   vmin  = np.log10(Imax*1e-4)

   #img   = ax[0].imshow(np.log10(Iim), extent=[-1,1,-1,1], cmap=cmap0, origin='lower', vmin=vmin, vmax=vmax)
   #cax0  = inset_axes(ax[0], width="5%", height="100%", loc="lower left",
   #                   bbox_to_anchor=(1.05, 0.0, 1, 1), bbox_transform=ax[0].transAxes, borderpad=0,)
   #cbar0 = fig.colorbar(img, cax=cax0)
