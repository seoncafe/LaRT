...
class AMRGrid:
    ...
    def __init__(self, boxlen=2.0, origin=None):
        ...
        # dict of FITS header cards written by refinement methods or the user.
        # Format: { 'KEYWORD': (value, 'comment string') }
        self.metadata = {}
    ...
    def refine_sphere_radial(self, cx, cy, cz, radius,
        ...
        Shells are processed from outermost (level_min) to innermost (level_max).
        Each call to refine_sphere is additive, so inner shells only add finer
        splits on top of the coarser outer-shell refinement.
        ...
        level_min : int
            Refinement level of the outermost shell.
        level_max : int
            Maximum refinement level at the innermost shell (center).
        spacing : {'linear', 'log'}
            How shell-boundary radii are distributed between ``radius`` and 0.

            ``'linear'`` — equal-width shells:
        ...
        ratio : float
            Radius reduction factor per level for ``spacing='log'``.
            Default 0.5 → each inner shell has half the radius of the outer one.
        custom_radii : list of float or None
            If provided, overrides ``spacing`` and ``ratio``.  Must have exactly
            ``level_max - level_min + 1`` entries, ordered from largest to
            smallest.  Each entry is the outer boundary of the corresponding
            refinement shell.
        refine_boundary : bool, optional (default False)
            If True, every leaf cell that intersects the sphere surface
            (radius) is additionally forced to ``boundary_level_max``,
            ensuring a sharply resolved geometric boundary on top of the
            radial shell structure.  Increases cell count near the surface.
        boundary_level_max : int or None, optional (default None)
            Maximum refinement level applied to sphere-surface cells when
        ...
        Example
        -------
        Sphere of radius 50, 4 shells, linear spacing:

            grid.refine_sphere_radial(0, 0, 0, 50,
        ...
        Resulting shell structure (radius, level):
            ...
            r > 50    →  unchanged  (background)

        Log spacing with ratio=0.5 and the same parameters:
            r < 6.25  →  level 4
            6.25–12.5 →  level 3
            12.5–25   →  level 2
            25–50     →  level 1
        ...
        if custom_radii is not None:
            if len(custom_radii) != n_shells:
                raise ValueError(
                    f'custom_radii must have {n_shells} entries '
                    f'(one per level from level_min={level_min} to '
                    f'level_max={level_max}), got {len(custom_radii)}'
                )
            radii = list(custom_radii)
        elif spacing == 'linear':
            radii = [radius * (n_shells - i) / n_shells
                     for i in range(n_shells)]
        elif spacing == 'log':
            radii = [radius * (ratio ** i) for i in range(n_shells)]
        else:
        ...
        def _target(d):
            ...
            # matching shell is the finest (highest level) the point is inside.
            t = 0
        ...
        def _recurse(cell):
            ...
            dy = max(cell.ymin - cy, 0.0, cy - cell.ymax)
            dz = max(cell.zmin - cz, 0.0, cz - cell.zmax)
            t_cls = _target((dx*dx + dy*dy + dz*dz) ** 0.5)
            if max(t_cen, t_cls) > cell.level:
                cell.split()
                for child in cell.children:
                    _recurse(child)
        ...
        if refine_boundary:
            ...
            self.refine_geometry(_inside, _intersects, blmax,
