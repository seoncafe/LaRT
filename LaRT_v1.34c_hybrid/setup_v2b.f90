...
! read parameters
  ...
  par%Dfreq0    = vtherm0/(par%wavelength0*um2km)
  ...
  if (mpar%p_rank == 0) then
     ...
     write(*,'(a,es12.3)') 'Dust-to_Gas Ratio         : ', par%DGR
  ...
  if (len_trim(scatt_mat_file) > 0) then
     ...
     par%albedo  = albedo
     par%hgg     = hgg
     ! par%wavelength0 and wavelength is in units of micron.
     par%wavelength0 = wavelength
     read(unit,*) string_tmp
  ...
  real(kind=wp) :: freq,prof,Dfreq_ref,vtherm_ref, tmp
  ...
  if (len_trim(line_prof_file) > 0) then
     ...
     Dfreq_ref  = vtherm_ref/(par%wavelength0*um2km)
     ...
     !--- speedc is given in km and wavelength_LyaH is given in units of micron.
     if (par%line_prof_file_type == 0) then
        xfreq(:) = (xfreq(:) - speedc/(par%wavelength0*um2km))/Dfreq_ref
     else if (par%line_prof_file_type == 1) then
        xfreq(:) = -(xfreq(:) - par%wavelength0*1e4)/(par%wavelength0*1e4)*(speedc/vtherm_ref)
     endif

     line_prof%nPDF = nfreq
     call create_mem(line_prof%xfreq, [line_prof%nPDF+1])
     call create_mem(line_prof%PDF,   [line_prof%nPDF])
     call create_mem(line_prof%alias, [line_prof%nPDF])
  ...
  subroutine setup_MPI_openMP_parameters()
     ...
     !$OMP PARALLEL default(shared)
     mpar%num_threads(mpar%p_rank+1) = omp_get_num_threads()
     !$OMP END PARALLEL
