...
contains
  ...
  if (mpar%h_rank == 0) then
     ...
     do i=1, grid%nz
        ...
        call interp(xarr,yarr,xnew,ynew,left=0.0_wp,right=0.0_wp)
        array(:,:,i) = ynew
     enddo
  ...
  if (mpar%h_rank == 0) then
     ...
     do i=1, grid%nx
        ...
        if (r >= x1 .and. r <= x2) then
           call interp(xarr,yarr,r,ynew,left=0.0_wp,right=0.0_wp)
           array(i,j,k) = ynew
        endif
     enddo
     enddo
  ...
  integer       :: unit, info, i, sample_method, ierr
  ...
  call MPI_BCAST(emiss%nPDF, 1, MPI_INTEGER, 0, mpar%hostcomm, ierr)
  call create_shared_mem(emiss%axis,       [emiss%nPDF])
  call create_shared_mem(emiss%prob,       [emiss%nPDF])
  call create_shared_mem(emiss%prob_alias, [emiss%nPDF-1])
  call create_shared_mem(emiss%alias,      [emiss%nPDF-1])
  ...
  if (mpar%h_rank == 0) then
     ...
     if (xarr(emiss%nPDF) > xmax) then
        ...
        call interp(xarr,yarr,xmax,emiss%prob(emiss%nPDF),left=0.0_wp,right=0.0_wp)
     endif
     if (allocated(xarr)) deallocate(xarr)
     if (allocated(yarr)) deallocate(yarr)
  ...
  if (sample_method > 0) then
     call create_shared_mem(emiss%wgt, [emiss%nPDF])
     if (mpar%h_rank == 0) then
        !-- we need to deal with the case where x-axis values are not equally spaced.
  ...
  call MPI_BCAST(star%nstars, 1, MPI_INTEGER, 0, mpar%hostcomm, ierr)

  call create_shared_mem(star%x,     [star%nstars])
  call create_shared_mem(star%y,     [star%nstars])
  call create_shared_mem(star%z,     [star%nstars])
  call create_shared_mem(star%lum,   [star%nstars])
  call create_shared_mem(star%prob,  [star%nstars])
  call create_shared_mem(star%alias, [star%nstars])
  ...
  if (sample_method > 0) then
     call create_shared_mem(star%wgt,  [star%nstars])
     if (mpar%h_rank == 0) then
