...
!DIR$ ATTRIBUTES INLINE :: voigt_mod_voigt
  ...
  if (par%DGR > 0.0_wp) then
     ...
     DnuHK  = grid%DnuHK_ref_half/grid%Dfreq_ratio(i1,i2,i3)
    ...
    if (.not. par%use_reduced_wgt) then
       if (rand_number() > par%albedo) then
         ...
         if (par%save_Jabs) then
            ...
            xfreq_ref = (photon%xfreq + uu1)*grid%Dfreq_ratio(photon%icell,photon%jcell,photon%kcell)
...
#ifdef PEELINGOFF
    !--- Peeling-off
    call peeling_dust_stokes(photon,grid)
#endif

    !--- Select a new cos(theta) from the numerical table.
    Prand = rand_number()
    call interp_eq(scatt_mat%phase_CDF,scatt_mat%cosp,Prand,cost)
...
!DIR$ ATTRIBUTES INLINE :: voigt_mod_voigt
...
#ifdef CALCP
    grid%Pa(photon%icell,photon%jcell,photon%kcell) = grid%Pa(photon%icell,photon%jcell,photon%kcell)+photon%wgt
#endif
    ...
    call peeling_resonance_stokes(photon,grid,xfreq_atom,[ux,uy,uz])
#endif
...
#ifdef PEELINGOFF
  call peeling_dust_nostokes(photon,grid)
#endif
...
!DIR$ ATTRIBUTES INLINE :: voigt_mod_voigt
  ...
  photon%nscatt_HI = photon%nscatt_HI + 1
#ifdef CALCP
  grid%Pa(i1,i2,i3) = grid%Pa(i1,i2,i3) + photon%wgt
#endif
  ...
  if (par%recoil) then
     g_recoil     = g_recoil0 /(grid%Dfreq(i1,i2,i3)*par%atom_no)
     photon%xfreq = photon%xfreq - g_recoil * (1.0_wp - cost)
  endif
#ifdef PEELINGOFF
