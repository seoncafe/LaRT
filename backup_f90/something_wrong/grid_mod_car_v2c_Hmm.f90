module grid_mod
  ...
  implicit none
  real(kind=wp), pointer :: Temp(:,:,:) => null()
  real(kind=wp), pointer :: xx(:) => null(), yy(:) => null(), zz(:) => null()
contains
  !----------------------------------------
...
!  real(kind=wp), pointer :: xx(:) => null(), yy(:) => null(), zz(:) => null()
  ...
  !call loop_divide(grid%nx,mpar%h_nproc,mpar%h_rank,loop1,loop2)
  !do i=loop1,loop2
  !   grid%xface(i) = (i-1)*grid%dx + grid%xmin
  !enddo
  !call loop_divide(grid%ny,mpar%h_nproc,mpar%h_rank,loop1,loop2)
  !do j=loop1,loop2
  !   grid%yface(j) = (j-1)*grid%dy + grid%ymin
  !enddo
  !call loop_divide(grid%nz,mpar%h_nproc,mpar%h_rank,loop1,loop2)
  !do k=loop1,loop2
  !   grid%zface(k) = (k-1)*grid%dz + grid%zmin
  !enddo
  ...
  call create_shared_mem(Temp, [grid%nx, grid%ny, grid%nz])
  call create_shared_mem(xx,   [grid%nx])
  call create_shared_mem(yy,   [grid%ny])
  call create_shared_mem(zz,   [grid%nz])
  ...
  grid%DnuHK_ref_half = DnuHK_Hz_half
#endif
  ...
  do loop = loop1,loop2
     ...
     vtherm                  = 0.12843374_wp*sqrt(Temp(i,j,k))/sqrt(1.0_wp*par%atom_no)
  ...
  if (len_trim(par%dens_file) > 0) then
     ...
     do loop = loop1, loop2
        call array_3D_indices(grid%nx,grid%ny,grid%nz,loop,i,j,k)
        grid%rhokap(i,j,k) = 1.0_wp
        if (par%DGR > 0.0_wp) grid%rhokapD(i,j,k) = par%cext_dust * par%DGR
     enddo
  endif
  !call MPI_BARRIER(mpar%hostcomm, ierr)
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
...
!  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
!print*,'c',mpar%p_rank,opacity_sum,opac_norm
!  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
!  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
!print*,'0',mpar%p_rank,sum(grid%rhokap)
!  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
     do loop = loop1, loop2
        call array_3D_indices(grid%nx,grid%ny,grid%nz,loop,i,j,k)
        grid%rhokap(i,j,k) = grid%rhokap(i,j,k) * opac_norm
        if (par%DGR > 0.0_wp) grid%rhokapD(i,j,k) = grid%rhokapD(i,j,k) * opac_norm
     enddo
!  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
!print*,'1',mpar%p_rank,sum(grid%rhokap)
!  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
     ...
     enddo
     call MPI_ALLREDUCE(MPI_IN_PLACE, opacity_sum, 1, MPI_DOUBLE_PRECISION, MPI_SUM, mpar%hostcomm, ierr)
     call MPI_ALLREDUCE(MPI_IN_PLACE, nopac,       1, MPI_DOUBLE_PRECISION, MPI_SUM, mpar%hostcomm, ierr)
  ...
  if (par%save_input_grid .and. mpar%p_rank == 0) then
     ...
     Temp(:,:,:) = grid%rhokap(:,:,:)*grid%Dfreq(:,:,:)/par%cross0 / par%distance2cm
