...
contains
  ...
  do while(.true.)
     ...
     if (cos_ang >= 0.0_wp .and. det >= 0.0_wp) then
        ...
        !-- 2021.10.01, The cost factor is natrally taken into account. No extra cost factor is not required.
        !--- a factor to calculate the flux (luminosity) incident onto the planet surface.
        !x  = photon%x
        !y  = photon%y
        !z  = photon%z + par%distance_star_to_planet
        !rr = sqrt(x**2 + y**2 + z**2)
        !photon%flux_factor = flux_fac1 * (x*photon%kx + y*photon%ky + z*photon%kz) / rr * photon%wgt
  ...
  !$OMP THREADPRIVATE(parameters_initialized, r1,r2, tot_omega_S,flux_fac1, cos_vt1,cos_vt2,cos_vt3, cos_vt, Prob, Palias, wgt, alias)
  ...
  if (cos_ang >= 0.0_wp .and. det >= 0.0_wp) then
     ...
     !--- Uniformly Intensity if (par%stellar_limb_darkening <= 0)
     !--- Uniform Disk (Lambert-like?)
     ...
     !z  = photon%z + par%distance_star_to_planet
     !rr = sqrt(x**2 + y**2 + z**2)
     !photon%flux_factor = ((x*photon%kx + y*photon%ky + z*photon%kz) / rr) * flux_fac1 * photon%wgt
     photon%flux_factor = flux_fac1 * photon%wgt
#ifdef TEST
     write((mpar%p_rank+1)*1000 +my_threadid+1, '(11ES15.7)') cosvt, vphi, cost, phi, photon%wgt,&
                                                              photon%x,photon%y,photon%z,photon%kx,photon%ky,photon%kz
  ...
  if (par%stellar_limb_darkening <= 0) then
     ran_cost = rand_number()
  else if (par%stellar_limb_darkening == 1) then
     ran_cost = sqrt(rand_number())
     !!-- test2
     !!ran_cost = rand_number()
     !!pobs%wgt = 2.0d0 * ran_cost
  else if (par%stellar_limb_darkening == 2) then
     ran_cost = rand_eddington_limb_darkening()
     !!-- test1
     !!ran_cost = sqrt(rand_number())
     !!pobs%wgt = 0.75d0*ran_cost + 0.5d0
     !!-- test2
     !!ran_cost = rand_number()
     !!pobs%wgt = ran_cost*(1.5d0*ran_cost + 1.0d0)
  else
     ran_cost = rand_general_limb_darkening()
     !!-- test1
     !!ran_cost = sqrt(rand_number())
     !!pobs%wgt = w_general_limb_darkening(ran_cost)
     !!-- test2
     !!ran_cost = rand_number()
     !!pobs%wgt = f_general_limb_darkening(ran_cost)
  endif

  cosvt = ran_cost*sqrt(1.0_wp - cosvt0**2 + (cosvt0*ran_cost)**2) + cosvt0*(1.0_wp - ran_cost**2)
  sinvt = sqrt(1.0_wp - cosvt**2)
  vphi  = twopi*rand_number()
  cosvp = cos(vphi)
  sinvp = sin(vphi)
