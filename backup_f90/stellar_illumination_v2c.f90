...
contains
  ...
  type(photon_type), intent(inout) :: photon
  integer       :: i
  real(kind=wp) :: cos_ang, fsum1, fsum2
  real(kind=wp) :: cosvt, sinvt, vphi, cosvp, sinvp
  real(kind=wp) :: cost, sint, phi, cosp, sinp
  ...
  if (.not. parameters_initialized) then
     ...
     !-- bug-fixed (2021.10.05)
     !-- flux_fac1 depends on the limb darkening function.
  ...
  do while(.true.)
     ...
     if (cos_ang >= 0.0_wp .and. det >= 0.0_wp) then
        ...
        if (par%stellar_limb_darkening <= 0) then
           photon%wgt = 1.0_wp
        !--- Uniform Disk when viewed by an observer
        else if (par%stellar_limb_darkening == 1) then
        ...
        !-- bug-fixed (2021.10.05)
        photon%flux_factor = flux_fac1 * cos_ang * photon%wgt
...
!#endif
  ...
  if (cos_ang >= 0.0_wp .and. det >= 0.0_wp) then
     ...
     !--- No weight change for the Uniformly Intensity case (par%stellar_limb_darkening <= 0)
     !--- Uniform Disk when viewed by an observer
