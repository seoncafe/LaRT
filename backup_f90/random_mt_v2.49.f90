...
!-----
  ...
  public random_alias_setup, rand_alias_choise, rand_alias_constant, rand_alias_linear
...
!==============================
  ...
  !------------------------------
  function rand_alias_constant64(prob_alias, alias, xaxis, idx_out) result(xresult)
  ...
  xresult = (xaxis(idx+1) - xaxis(idx)) * rand_number() + xaxis(idx)
  if (present(idx_out)) idx_out = idx
  end function rand_alias_constant64
  !------------------------------
  function rand_alias_constant32(prob_alias, alias, xaxis, idx_out) result(xresult)
  !--- requirement : size(prob_alias) = size(alias), size(xaxis) = size(probs_alias) + 1.
  implicit none
  ...
  !------------------------------
  function rand_alias_linear64(prob_alias, alias, xaxis, prob, idx_out) result(xresult)
  ...
  xresult = (sqrt(prob(idx)**2 + (prob(idx+1)**2 - prob(idx)**2)*rand_number()) - prob(idx)) &
            * (xaxis(idx+1)-xaxis(idx))/(prob(idx+1)-prob(idx)) + xaxis(idx)
  if (present(idx_out)) idx_out = idx
  end function rand_alias_linear64
  !------------------------------
  function rand_alias_linear32(prob_alias, alias, xaxis, prob, idx_out) result(xresult)
  !--- requirement : size(prob_alias) = size(alias), size(prob) = size(xaxis) = size(probs_alias) + 1.
  implicit none
  real(real32),      intent(in)  :: prob_alias(:)
  integer,           intent(in)  :: alias(:)
