...
contains
  ...
  if (len_trim(par%emiss_file) > 0) then
     ...
     case ('fits','density1','density2')
        ...
        if (mpar%h_rank == 0) then
           select case(trim(par%emiss_file))
           case ('density1')
              !-- when the emissivity is proportional to density.
              call read_3D(trim(par%dens_file),grid%Pem,reduce_factor=par%reduce_factor,centering=par%centering)
           case ('density2')
