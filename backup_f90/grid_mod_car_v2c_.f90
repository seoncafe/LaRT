...
contains
  ...
  if (mpar%h_rank == 0) then
     ...
     if (par%taumax > 0.0_wp) then
        ...
        do i=1, grid%nx
           if (grid%rhokap(i,j,k) > 0.0_wp) then
              opacity_sum = opacity_sum + grid%rhokap(i,j,k)*voigt(0.0_wp,grid%voigt_a(i,j,k))
              nopac       = nopac + 1
           endif
        enddo
        enddo
        ...
        do i=1, grid%nx
           if (grid%rhokap(i,j,k) > 0.0_wp) then
              opacity_sum = opacity_sum + grid%rhokap(i,j,k)*grid%Dfreq(i,j,k)
              nopac       = nopac + 1
           endif
        enddo
        enddo
     ...
     if (par%DGR  > 0.0_wp) then
        ...
        do i=1, grid%nx
           if (grid%rhokap(i,j,k) > 0.0_wp) then
              opacity_sum = opacity_sum + grid%rhokapD(i,j,k)
              nopac       = nopac + 1
           endif
        enddo
        enddo
