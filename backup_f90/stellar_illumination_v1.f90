...
contains
  ...
  do while(.true.)
     ...
     if (cos_ang >= 0.0_wp .and. det >= 0.0_wp) then
        ...
        !TEST---write((mpar%rank+1) * 1000 + my_threadid+1, '(4ES15.7)') cosvt, vphi, cost, phi
        exit
     else
  ...
  real(wp), allocatable, save :: cos_vt(:), Prob(:), Palias(:)
  integer,  allocatable, save :: alias(:)
  !$OMP THREADPRIVATE(parameters_initialized, r1, r2, tot_omega_S, cos_vt1, cos_vt2, cos_vt3, cos_vt, Prob, Palias, alias)
  ...
  if (.not. parameters_initialized) then
     ...
     do i=1, nvtheta
        ...
        !TEST---if (mpar%rank == 0 .and. my_threadid == 1) then
        !TEST---   write(101,'(2ES15.7)') cos_vt(i), Prob(i)
        !TEST---endif
     enddo
     !-- probability distribution function for cosvt
  ...
  !-- (vartheta, varphi) = angles where the photon is injected on the stellar surface (with respect to the star.)
  !-- assume that Prob is a linear function between cos_vt(idx) and cos_vt(idx+1).
  ...
  if (cos_ang >= 0.0_wp .and. det >= 0.0_wp) then
     ...
     z  = photon%z + par%distance_star_to_planet
     rr = sqrt(x**2 + y**2 + z**2)
     photon%flux_factor = (x*photon%kx + y*photon%ky + z*photon%kz) / rr * (tot_omega_S/twopi)
  else
  ...
  real(kind=wp) :: xx,yy,zz,kx,ky,kz
  ...
  cosvp  = cos(vphi)
  sinvp  = sin(vphi)
  xx     = par%stellar_radius * sinvt * cosvp
  yy     = par%stellar_radius * sinvt * sinvp
  zz     = par%stellar_radius * cosvt - par%distance_star_to_planet
