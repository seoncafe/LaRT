...
contains
  ...
  do i=1,par%nobs
    ...
    !--- (2021.05.16)
    if (trim(par%source_geometry) == 'plane_illumination' .and. (observer(i)%x /= 0.0_wp .or. observer(i)%y /= 0.0_wp)) then
      exit
    endif
  ...
  do i=1, par%nobs
    ...
    if (ix >= 1 .and. ix <= observer(i)%nxim .and. iy >= 1 .and. iy <= observer(i)%nyim) then
       ...
       xfreq_ref = xfreq_ref * grid%Dfreq_ratio(photon%icell,photon%jcell,photon%kcell)
       ixf       = floor((xfreq_ref - grid%xfreq_min)/grid%dxfreq) + 1
       if (ixf < 1 .or. ixf > grid%nxfreq) exit
       !--- bug-fixed: if (ixf < 1 .or. ixf > grid%nxfreq) return
#ifdef FINE_STRUCTURE
