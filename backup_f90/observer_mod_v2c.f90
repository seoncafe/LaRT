...
!---------------------------------------------------------------------------------
  ...
  if (is_finite(par%obsx(1)) .and. is_finite(par%obsy(1)) .and. is_finite(par%obsz(1))) then
     ...
     do i=1, par%nobs
        ...
        !-- A serious numerical unstable error!
        !-- This numerical error causes all rays to be missed from the detector plane when par%distance is very big (2021.09.22).
        ...
        if (sinb(i) == 0.0_wp) then
           ...
           cosa(i) = (observer(i)%x - par%rotation_center_x)/(par%distance*sinb(i))
           sina(i) = (observer(i)%y - par%rotation_center_y)/(par%distance*sinb(i))
        endif
        cosg(i) = cos(par%gamma(i)*deg2rad)
        sing(i) = sin(par%gamma(i)*deg2rad)

        !-- 2021.09.06, bug-fixed
        par%alpha(i)         = atan2(sina(i), cosa(i))*rad2deg
