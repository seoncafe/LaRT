...
!--- d   = cumulative path length
  ...
  do while(.true.)
     !rhokap = grid%rhokap(icell,jcell,kcell)*voigt(xfreq, grid%voigt_a(icell,jcell,kcell))
     !=DnuHK  = grid%DnuHK_ref/grid%Dfreq_ratio(icell,jcell,kcell)
     !=rhokap = grid%rhokap(icell,jcell,kcell)*(voigt(xfreq,       grid%voigt_a(icell,jcell,kcell))/3d0 + &
     !=                                         voigt(xfreq-DnuHK, grid%voigt_a(icell,jcell,kcell))*(2d0/3d0))
     DnuHK  = grid%DnuHK_ref_half/grid%Dfreq_ratio(icell,jcell,kcell)
  ...
  subroutine raytrace_to_edge_tau_HI(photon0,grid,tau_HI)
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_tau_HI

  subroutine raytrace_to_edge_column(photon0,grid,N_HI,tau_dust)
!--- Find the cumulative optical depth due to HI and dust, column density of HI to the edge of grid. (2020.10.21)
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_column
...
!--- integrate through grid
  do while(photon%inside)
     ...
     !rhokap = grid%rhokap(icell,jcell,kcell)*voigt(photon%xfreq, grid%voigt_a(icell,jcell,kcell))
     !=DnuHK  = grid%DnuHK_ref/grid%Dfreq_ratio(icell,jcell,kcell)
     !=rhokap = grid%rhokap(icell,jcell,kcell)*(voigt(photon%xfreq,       grid%voigt_a(icell,jcell,kcell))/3d0 + &
...
#ifdef CALCJ
     !ix = floor((photon%xfreq - grid%xfreq_min)/grid%dxfreq)+1
     ! 2017-06-20, updated to take into account inhomogeneous temperature distribution.
     photon%xfreq_ref = photon%xfreq * grid%Dfreq_ratio(iold,jold,kold)
     ix = floor((photon%xfreq_ref - grid%xfreq_min)/grid%dxfreq)+1
     if (ix > 0 .and. ix <= grid%nxfreq .and. &
         iold > 0 .and. iold <= grid%nx .and. jold > 0 .and. jold <= grid%ny .and. kold > 0 .and. kold <= grid%nz) then
        if (grid%rhokap(iold,jold,kold) > 0.0_wp) then
           !grid%J(ix,iold,jold,kold) = grid%J(ix,iold,jold,kold) + (d - dold) * photon%wgt
           grid%J(ix,iold,jold,kold) = grid%J(ix,iold,jold,kold) + del * photon%wgt
        endif
     endif
#endif
#ifdef CALCPnew
     if (iold > 0 .and. iold <= grid%nx .and. jold > 0 .and. jold <= grid%ny .and. kold > 0 .and. kold <= grid%nz) then
        if (grid%rhokap(iold,jold,kold) > 0.0_wp) then
           grid%Pa_new(iold,jold,kold) = grid%Pa_new(iold,jold,kold) + dtauH * photon%wgt
        endif
     endif
#endif
...
#ifdef CALCJ
  !ix = floor((photon%xfreq - grid%xfreq_min)/grid%dxfreq)+1
  photon%xfreq_ref = photon%xfreq * grid%Dfreq_ratio(icell,jcell,kcell)
  ix = floor((photon%xfreq_ref - grid%xfreq_min)/grid%dxfreq)+1
  if (ix > 0 .and. ix <= grid%nxfreq .and. &
     ...
     if (grid%rhokap(icell,jcell,kcell) > 0.0_wp) then
        !grid%J(ix,icell,jcell,kcell) = grid%J(ix,icell,jcell,kcell) + (d - dold) * photon%wgt
  ...
  if (icell > 0 .and. icell <= grid%nx .and. jcell > 0 .and. jcell <= grid%ny .and. kcell > 0 .and. kcell <= grid%nz) then
     if (grid%rhokap(icell,jcell,kcell) > 0.0_wp) then
        grid%Pa_new(icell,jcell,kcell) = grid%Pa_new(icell,jcell,kcell) + dtauH * photon%wgt
     endif
  endif
#endif
...
!--- xp, yp, zp = the current photon coordinates.
  ...
  if (ix > 0 .and. ix <= grid%nxfreq .and. &
      icell > 0 .and. icell <= grid%nx .and. jcell > 0 .and. jcell <= grid%ny .and. kcell > 0 .and. kcell <= grid%nz) then
     !if(d < dold) print*,'warning: raytrace_car...', d,dold,d_overshoot,photon%inside
     !bug-fixed, 2017-06-20
     !if (grid%rhokap(iold,jold,kold) > 0.0_wp) then
     if (grid%rhokap(icell,jcell,kcell) > 0.0_wp) then
        !grid%J(ix,icell,jcell,kcell) = grid%J(ix,icell,jcell,kcell) + (d - dold) * photon%wgt
        grid%J(ix,icell,jcell,kcell) = grid%J(ix,icell,jcell,kcell) + del * photon%wgt
...
!--- xp, yp, zp = the current photon coordinates.
  ...
  subroutine raytrace_to_tau_car_xyper_atmosphere(photon,grid,tau_in)
!--- Find the coordinates and cell indices corresponding to an input optical depth tau_in.
!---
!--- The algorithm is based on the fast Voxel traversal algorithm
!--- by Amanatides & Woo (1987), Proc. of Eurographics, 87, 3
...
!--- xp, yp, zp = the current photon coordinates.
  ...
  return
  end subroutine raytrace_to_tau_car_xyper_atmosphere
