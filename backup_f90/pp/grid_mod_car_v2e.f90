...
contains
  ...
  grid%Dfreq_ref      = line%vtherm1*sqrt(par%temperature)/(line%lambda0*um2km)
#ifdef FINE_STRUCTURE
  grid%DnuHK_ref      = line%DnuHK_Hz
#endif
  ...
  if (mpar%h_rank == 0) then
     ...
     do i=1,grid%nx
        ...
        grid%Dfreq(i,j,k)       = vtherm/(line%lambda0*um2km)
        grid%voigt_a(i,j,k)     = (line%A21/fourpi)/grid%Dfreq(i,j,k)
     enddo
     enddo
     enddo
  endif
  ...
  if (mpar%h_rank == 0) then
     ...
     !-- to mask the locations where photons are supposed to be destroyed.
     if (trim(par%geometry) == 'spherical_atmosphere') then
        do k=1,grid%nz
        do j=1,grid%ny
        do i=1,grid%nx
           rr = sqrt(xx(i)**2 + yy(j)**2 + zz(k)**2)
           if (rr <= grid%rmin) grid%rhokap(i,j,k) = -999.9_wp
        enddo
        enddo
        enddo
     endif
  ...
  if (len_trim(par%emiss_file) > 0) then
     if (trim(par%geometry) == 'plane_atmosphere') then
        ...
        if (par%sampling_method == 0) then
           ...
           call create_shared_mem(grid%alias, [size(grid%Pem)])
           grid%Pem1D(1:size(grid%Pem))  => grid%Pem
  ...
  if (par%save_input_grid .and. mpar%p_rank == 0) then
     ...
     Temp(:,:,:) = grid%rhokap(:,:,:)*grid%Dfreq(:,:,:)/line%cross0 / par%distance2cm
  ...
  grid%voigt_amean = (line%A21/fourpi)/grid%Dfreq_ref
  ...
  if (is_finite(par%lambda_min) .and. is_finite(par%lambda_max)) then
     ...
     par%xfreq_min = -(par%lambda_max - line%lambda0*1e4)/(line%lambda0*1e4)*(speedc/vtherm)
     par%xfreq_max = -(par%lambda_min - line%lambda0*1e4)/(line%lambda0*1e4)*(speedc/vtherm)
  ...
  if (.not.(is_finite(par%xfreq_max) .and. is_finite(par%xfreq_min))) then
     ...
     if (par%Vexp == 0.0_wp) then
        ...
        par%xfreq_min  = -(floor(xscale * par%atau3 + grid%DnuHK_ref/grid%Dfreq_ref)+1)
     else if (par%Vexp > 0.0_wp) then    ! expanding
        vtherm         = line%vtherm1*sqrt(par%temperature)
        par%xfreq_max  = floor(xscale * par%atau3)+1
        par%xfreq_min  = -(floor(xscale * par%atau3 + abs(par%Vexp)/vtherm + grid%DnuHK_ref/grid%Dfreq_ref)+1)
  ...
  if (mpar%h_rank == 0) then
     ...
     grid%dlambda     =  line%vtherm1*sqrt(par%temperature) / speedc * (line%lambda0 * 1e4_wp) * grid%dxfreq
     grid%lambda(:)   = (grid%velocity(:)/speedc + 1.0_wp) * (line%lambda0 * 1e4_wp)
  endif
