...
!---------------------------------------------------------------------------
!  Private helper: initialise voxel-traversal parameters (standard geometry).
!  Called by raytrace_to_dist_* and raytrace_to_edge_* subroutines.
!  Returns out_of_grid = .true. when the photon is already leaving the grid;
!  in that case the caller should return immediately (or set photon%%inside).
!---------------------------------------------------------------------------
  subroutine setup_traversal_car(grid, xp, yp, zp, kx, ky, kz, &
                                  icell, jcell, kcell,            &
                                  istep, jstep, kstep,            &
                                  tx, ty, tz, delx, dely, delz,   &
                                  out_of_grid)
  use define
  implicit none
  type(grid_type), intent(in)    :: grid
  real(kind=wp),   intent(in)    :: xp, yp, zp, kx, ky, kz
  integer,         intent(inout) :: icell, jcell, kcell
  integer,         intent(out)   :: istep, jstep, kstep
  real(kind=wp),   intent(out)   :: tx, ty, tz, delx, dely, delz
  logical,         intent(out)   :: out_of_grid

  out_of_grid = .false.

  !--- x-axis
  if (kx > 0.0_wp) then
     ! icell = nx+1 & xface(nx+1) = xp & kx > 0: photon leaving grid.
     if (icell > grid%nx .and. grid%xface(icell) <= xp) then
        out_of_grid = .true.; return
     endif
     istep = 1;  tx = (grid%xface(icell+1)-xp)/kx;  delx =  grid%dx/kx
  else if (kx < 0.0_wp) then
     if (grid%xface(icell) == xp) then
        if (icell > 1) then
           icell = icell - 1   ! skip zero-length step at face
        else
           ! icell = 1 & xface(1) = xp & kx < 0: photon leaving grid.
           out_of_grid = .true.; return
        endif
     endif
     istep = -1;  tx = (grid%xface(icell)-xp)/kx;  delx = -grid%dx/kx
  else
     istep = 0;  tx = hugest;  delx = hugest
  endif
  ...
  if (ky > 0.0_wp) then
     ...
     jstep = 1;  ty = (grid%yface(jcell+1)-yp)/ky;  dely =  grid%dy/ky
     ...
     jstep = -1;  ty = (grid%yface(jcell)-yp)/ky;  dely = -grid%dy/ky
  else
     jstep = 0;  ty = hugest;  dely = hugest
  endif
  ...
  if (kz > 0.0_wp) then
     ...
     kstep = 1;  tz = (grid%zface(kcell+1)-zp)/kz;  delz =  grid%dz/kz
     ...
     kstep = -1;  tz = (grid%zface(kcell)-zp)/kz;  delz = -grid%dz/kz
  else
     kstep = 0;  tz = hugest;  delz = hugest
  endif

  end subroutine setup_traversal_car

  subroutine raytrace_to_dist_car(photon0,grid,dist_in,tau)
!--- Find the cumulative optical depth to an input distance dist_in.
!--- The algorithm is based on the fast Voxel traversal algorithm (Amanatides & Woo 1987, Proc. of Eurographics, 87, 3)
!--- Author: Kwang-Il Seon, 2023-01-07
...
! local variables
  ...
  real(kind=wp) :: rhokap, u1,u2,xfreq
  integer       :: idx_min
  logical       :: out_of_grid
...
!--- d   = cumulative path length
  ...
  subroutine raytrace_to_dist_tau_gas_car(photon0,grid,dist_in,tau_gas)
!--- Find the cumulative optical depth due to gas and dust, column density of gas to the edge of grid. (2020.10.21)
!--- The algorithm is based on the fast Voxel traversal algorithm (Amanatides & Woo 1987, Proc. of Eurographics, 87, 3)
!--- Author: Kwang-Il Seon, 2023-01-07
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_dist_tau_gas_car

  subroutine raytrace_to_dist_column_car(photon0,grid,dist_in,N_gas,tau_dust)
!--- Find the cumulative optical depth due to gas and dust, column density of gas to the edge of grid. (2020.10.21)
!--- The algorithm is based on the fast Voxel traversal algorithm (Amanatides & Woo 1987, Proc. of Eurographics, 87, 3)
!--- Author: Kwang-Il Seon, 2023-01-07
...
!--- d   = cumulative path length
  ...
  !--- integrate through grid
  do while(.true.)
     rho    = grid%rhokap(icell,jcell,kcell)*grid%Dfreq(icell,jcell,kcell) / line%cross0
  ...
  enddo
  return
  end subroutine raytrace_to_dist_column_car
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_car_xyzsym

  subroutine raytrace_to_edge_car_xysym(photon0,grid,tau)
!--- Find the cumulative optical depth to the edge of grid.
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_car_xysym

  subroutine raytrace_to_edge_car_xyper(photon0,grid,tau)
!--- Find the cumulative optical depth to the edge of grid.
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_car_xyper

  subroutine raytrace_to_edge_car_zonly(photon0,grid,tau)
!--- Find the cumulative optical depth to the edge of grid.
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_car_zonly

  subroutine raytrace_to_edge_car_tau_gas(photon0,grid,tau_gas)
!--- Find the cumulative optical depth due to gas and dust, column density of gas to the edge of grid. (2020.10.21)
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  enddo
  return
  end subroutine raytrace_to_edge_car_tau_gas

  subroutine raytrace_to_edge_car_column(photon0,grid,N_gas,tau_dust)
!--- Find the cumulative optical depth due to gas and dust, column density of gas to the edge of grid. (2020.10.21)
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  subroutine raytrace_to_edge_car_tau_gas_atmosphere(photon0,grid,tau_gas)
!--- Find the cumulative optical depth due to gas and dust, column density of gas to the edge of grid. (2020.10.21)
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  do while(.true.)
     ...
     !--- The Lya photon is assumed to be destroyed by planet's dense molecular layer.
     if (grid%mask(icell,jcell,kcell) == -1_int8) then
        tau_gas = ieee_value(0.0_wp, ieee_positive_inf)
        exit
     endif
  ...
  enddo
  return
  end subroutine raytrace_to_edge_car_tau_gas_atmosphere

  subroutine raytrace_to_edge_car_column_atmosphere(photon0,grid,N_gas,tau_dust)
!--- Find the cumulative optical depth due to gas and dust, column density of gas to the edge of grid. (2020.10.21)
!--- Note that photon0.(x0,y0,z0) and photon0.(icell0,jcell0,kcell0) do not change.
!---
!--- Basic algorithm is based on the fast Voxel traversal algorithm
...
!--- d   = cumulative path length
  ...
  do while(.true.)
     ...
     !--- The Lya photon is assumed to be destroyed by planet's dense molecular layer.
     if (grid%mask(icell,jcell,kcell) == -1_int8) then
        N_gas = ieee_value(0.0d0, ieee_positive_inf)
