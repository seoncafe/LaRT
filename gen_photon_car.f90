...
contains
  ...
  !=== AMR mode: position + frequency + injection ===
  if (par%use_amr_grid) then
     ...
     call setup_isotropic_injection_amr(grid, photon)
     il = photon%icell_amr
     ...
     if (trim(par%spectral_type) /= 'continuum') then
        ...
        case (2)
           ...
           if (rand_number() <= one_over_three) photon%xfreq = photon%xfreq - DnuHK
        case (3)
           p1 = line%f12(1)
           p2 = line%f12(2)
           p1 = p1/(p2 + p1)
           if (rand_number() > p1) then
              iup          = 2
              del_xfreq    = line%delE_Hz(2) / amr_grid%Dfreq(il)
        ...
        case (6)
           p1    = line%f12(1); p2 = line%f12(2); p3 = line%f12(3)
           ptot  = p1 + p2 + p3
           pcum1 = p1/ptot; pcum2 = (p1 + p2)/ptot
           xi = rand_number()
           if (xi < pcum1) then
              iup = 1
              ...
              iup          = 3
              del_xfreq    = line%delE_Hz(3) / amr_grid%Dfreq(il)
              photon%xfreq = photon%xfreq - del_xfreq
           endif
        end select
     endif
     ...
     case ('continuum')
        ...
        photon%xfreq = photon%xfreq / (amr_grid%Dfreq(il) / grid%Dfreq_ref)
     case ('voigt0')
        photon%xfreq = photon%xfreq + rand_voigt(par%voigt_a0) * par%Dfreq0 / amr_grid%Dfreq(il)
     case ('voigt')
        photon%xfreq = photon%xfreq + rand_voigt(amr_grid%voigt_a(il))
     case ('gaussian')
     ...
     if (par%save_Jin) then
        u1        = amr_grid%vfx(il)*photon%kx + amr_grid%vfy(il)*photon%ky + amr_grid%vfz(il)*photon%kz
        xfreq_lab = (photon%xfreq + u1) * (amr_grid%Dfreq(il) / grid%Dfreq_ref)
     ...
     if (par%save_peeloff) call peeling_direct(photon, grid)
     return
  end if
  ...
  case ('uniform_cylinder','cylinder')
     ...
     !endif
  case ('uniform')
     !if (trim(par%source_geometry) == 'uniform' .and. par%source_rmax <= 0.0_wp) then
  ...
  if (trim(par%spectral_type) /= 'continuum') then
     ...
     case (3)
        ...
        if (rand_number() > p1) then
           ...
           iup = 2
           del_xfreq    = line%delE_Hz(2)/grid%Dfreq(photon%icell,photon%jcell,photon%kcell)
           photon%xfreq = photon%xfreq - del_xfreq
        endif
     case (4)
     ...
     case (6)
        ...
        if (xi < pcum1) then
           ...
           iup = 3
           del_xfreq    = line%delE_Hz(3)/grid%Dfreq(photon%icell,photon%jcell,photon%kcell)
           photon%xfreq = photon%xfreq - del_xfreq
        endif
     endselect
  endif
  ...
  end subroutine setup_isotropic_injection_amr
