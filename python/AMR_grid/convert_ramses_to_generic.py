...
When ``--compute-physics`` is given, the script also pre-computes CIE neutral
fraction (xHI), electron density (n_e), Case B Lyman-alpha emissivity, and
optionally dust pseudo-number density (if ``--metallicity`` is specified).
These derived quantities are written as additional columns/datasets alongside
the base hydrodynamic variables.
...
# --- Physics computation functions (matching physics_amr_mod.f90) ---

def cie_neutral_fraction(T):
    """CIE neutral fraction from single-formula ionization/recombination rates."""
    T = np.maximum(T, 10.0)
    T4 = T / 1.0e4
    k_ionize = 5.84862e-9 * np.sqrt(T4) * np.exp(-15.78215 / T4)
    k_recomb = 4.13e-13 * T4**(-0.7131 - 0.0115 * np.log(T4))
    return k_recomb / (k_ionize + k_recomb)


def cie_neutral_fraction_table(nH, T):
    """CIE neutral fraction using Voronov (1997) + Verner & Ferland (1996) rates.

    Uses log-linear interpolation on a pre-computed table (log10(T) from 3.0
    to 8.0 in steps of 0.1).  nH is unused (CIE is density-independent) but
    included for future density-dependent extensions.

    Parameters
    ----------
    nH : array_like
        Hydrogen number density [cm^-3] (unused, reserved).
    T : array_like
        Gas temperature [K].
    ...
    logt_arr = np.arange(3.0, 8.01, 0.1)  # 51 points
    T_tab = 10.0**logt_arr

    # Voronov (1997) collisional ionization rate [cm^3/s]
    Gamma_HI = (5.85e-11 * np.sqrt(T_tab) * np.exp(-157809.1 / T_tab)
                / (1.0 + np.sqrt(T_tab / 1.0e5)))

    # Verner & Ferland (1996) Case A recombination rate [cm^3/s]
    T4 = T_tab / 1.0e4
    alpha_A = 4.309e-13 * T4**(-0.6166) / (1.0 + 0.6703 * T4**0.5300)

    xHI_tab = alpha_A / (Gamma_HI + alpha_A)
    log_xHI_tab = np.log10(np.maximum(xHI_tab, 1.0e-30))

    logT = np.log10(np.maximum(T, 1.0))
    log_xHI = np.interp(logT, logt_arr, log_xHI_tab)

    result = 10.0**log_xHI
    result = np.where(T <= 1.0e3, 1.0, result)
    result = np.clip(result, 0.0, 1.0)
    return result


def electron_density(nH, xHI):
    """Electron density from neutral fraction (hydrogen only)."""
    return nH * (1.0 - xHI)
...
def caseB_lya_emissivity(nH, T, xHI, ne):
    ...
    T = np.maximum(T, 10.0)
    lambda_HG = 315614.0 / T
    alpha_B = 2.753e-14 * lambda_HG**1.5 / (1.0 + (lambda_HG / 2.74)**0.407)**2.242
    Ta = np.maximum(T, 100.0)
    P_B = 0.686 - 0.106 * np.log10(Ta / 1e4) - 0.009 * (Ta / 1e4)**(-0.44)
    nHI = nH * xHI
    nHII = nH * (1.0 - xHI)
    emiss_recomb = P_B * alpha_B * ne * nHII
    q_coll = (6.58e-18 / T**0.185) * np.exp(-4.86e4 / T**0.895)
...
def write_generic_text(
    ...
    # Build list of extra columns and their names
    ...
    with filename.open("w", encoding="utf-8") as handle:
        # Write header comment if extra columns are present
        if extra_names:
            base_cols = "x  y  z  level  nH[cm^-3]  T[K]  vx[km/s]  vy[km/s]  vz[km/s]"
            handle.write(f"# {base_cols}  {'  '.join(extra_names)}\n")
        handle.write(f"{len(x)}  {boxlen:.12e}\n")
        for i in range(len(x)):
            line = (
                "{:.12e}  {:.12e}  {:.12e}  {:d}  {:.12e}  {:.12e}  "
                "{:.12e}  {:.12e}  {:.12e}".format(
                    x[i], y[i], z[i], int(level[i]),
                    nH[i], T[i], vx[i], vy[i], vz[i]
                )
            )
            for arr in extra_arrays:
                line += "  {:.12e}".format(arr[i])
            handle.write(line + "\n")
...
def write_generic_fits(
    ...
    # Append optional physics columns
    for col_name, col_arr in [("xHI", xHI), ("n_e", n_e),
                               ("emissivity", emissivity), ("ndust", ndust),
                               ("metallicity", metallicity)]:
        if col_arr is not None:
            bp = _auto_bitpix_col(col_arr) if bitpix == 0 else bitpix
            fmt = "1E" if bp == -32 else "1D"
            typed_arr = col_arr.astype(np.float32 if bp == -32 else np.float64)
            cols.append(fits.Column(name=col_name, format=fmt, array=typed_arr))

    primary = fits.PrimaryHDU()
    table   = fits.BinTableHDU.from_columns(cols, name="AMRGRID")
    table.header["CREATOR"]  = ("convert_ramses_to_generic.py", "Created by")
    ...
    table.header["ORIGINX"]  = (-0.5 * float(boxlen), "Box origin x (centered)")
    table.header["ORIGINY"]  = (-0.5 * float(boxlen), "Box origin y (centered)")
    table.header["ORIGINZ"]  = (-0.5 * float(boxlen), "Box origin z (centered)")
...
def write_generic_hdf5(
    ...
    Optional physics datasets (xHI, n_e, emissivity, ndust, metallicity) are
    appended when not None.
    """
    try:
        import h5py
    ...
    for col_name, col_arr in [("xHI", xHI), ("n_e", n_e),
                               ...
                               ("metallicity", metallicity)]:
        if col_arr is not None:
            float_cols.append((col_name, col_arr))
    ...
    with h5py.File(filename, "w", libver="latest", track_order=True) as f:
        ...
        g.attrs["CREATOR"]  = "convert_ramses_to_generic.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--compute-physics",
        action="store_true",
        help="Pre-compute CIE neutral fraction, electron density, and Case B "
             "Lyman-alpha emissivity. Optionally also dust density if "
             "--metallicity is given.",
    )
    parser.add_argument(
        "--metallicity",
        type=float,
        default=None,
        help="Gas metallicity (mass fraction, e.g. 0.0134 for solar). "
             "When given together with --compute-physics, computes dust "
             "pseudo-number density via the Laursen+09 prescription.",
    )
...
def main() -> None:
    ...
    boxlen_phys, _ = convert_positions_unit(np.array([info.boxlen_cm]), args.output_unit)
    boxlen_phys = float(boxlen_phys[0])

    # Octree-alignment fix for RAMSES nx > 1.  See LaRT_v2.10 converter for
    # the full rationale: native cell size is boxlen_phys/(nx*2^L), which
    # cannot align with a LaRT octree of step boxlen_phys/2^L' unless nx
    # is a power of 2.  We re-anchor the output to the smallest power-of-2
    # cubic sub-block of the nx^3 base grid that encloses every populated
    # leaf, then bump levels by log2(m) so cell sizes remain exact.
    if info.nx != info.ny or info.ny != info.nz:
        raise SystemExit(
            f"non-cubic base grid (nx={info.nx}, ny={info.ny}, nz={info.nz}) "
            f"is not supported.")
    nx_base = info.nx
    if nx_base != 1:
        base_cell_phys = boxlen_phys / nx_base
        ix = np.floor(x_out / base_cell_phys).astype(np.int64)
        iy = np.floor(y_out / base_cell_phys).astype(np.int64)
        iz = np.floor(z_out / base_cell_phys).astype(np.int64)
        ix = np.clip(ix, 0, nx_base - 1)
        iy = np.clip(iy, 0, nx_base - 1)
        iz = np.clip(iz, 0, nx_base - 1)
        ix_min, iy_min, iz_min = int(ix.min()), int(iy.min()), int(iz.min())
        ix_max, iy_max, iz_max = int(ix.max()), int(iy.max()), int(iz.max())
        extent = max(ix_max - ix_min + 1,
                     iy_max - iy_min + 1,
                     iz_max - iz_min + 1)
        m = 1
        while m < extent:
        ...
        if m > nx_base:
            raise SystemExit(
                f"populated extent {extent} base cells exceeds RAMSES base "
                f"grid nx={nx_base}; cannot fit on a power-of-2 sub-block.")

        def _choose_origin(lo: int, hi: int, m: int, nx: int) -> int:
            cen = 0.5 * (lo + hi)
            o = int(round(cen - 0.5 * (m - 1)))
            o = max(o, hi - m + 1)
            o = min(o, lo)
            o = max(0, o)
            o = min(o, nx - m)
        ...
        sub_ix = _choose_origin(ix_min, ix_max, m, nx_base)
        sub_iy = _choose_origin(iy_min, iy_max, m, nx_base)
        sub_iz = _choose_origin(iz_min, iz_max, m, nx_base)
        ok = ((ix >= sub_ix) & (ix < sub_ix + m) &
              (iy >= sub_iy) & (iy < sub_iy + m) &
              (iz >= sub_iz) & (iz < sub_iz + m))
        if not bool(ok.all()):
            raise SystemExit(
                f"internal error: populated cells extend outside the chosen "
                f"sub-block (m={m}, origin=({sub_ix},{sub_iy},{sub_iz}))")
        x_out = x_out - sub_ix * base_cell_phys
        y_out = y_out - sub_iy * base_cell_phys
        z_out = z_out - sub_iz * base_cell_phys
        boxlen_out = m * base_cell_phys
        level_bump = int(round(math.log2(m)))
        if level_bump > 0:
            data["level"] = data["level"] + np.int32(level_bump)
        print(f"  RAMSES nx={nx_base} > 1: shrinking BOXLEN to a {m}x{m}x{m} "
              f"base-cell block (BOXLEN: {boxlen_phys:.4g} -> {boxlen_out:.4g} "
              f"{args.output_unit})")
        print(f"  populated bbox (base cells): "
              f"[{ix_min},{ix_max}] x [{iy_min},{iy_max}] x [{iz_min},{iz_max}]")
        print(f"  sub-block origin (base cells): ({sub_ix}, {sub_iy}, {sub_iz}); "
              f"level shift: +{level_bump}")
    else:
        boxlen_out = boxlen_phys

    # Shift to LaRT's centered convention ([-boxlen/2, +boxlen/2]).
    half_box = 0.5 * boxlen_out
    x_out = x_out - half_box
    y_out = y_out - half_box
    z_out = z_out - half_box

    # Compute optional physics quantities
    phys_xHI: Optional[np.ndarray] = None
    phys_ne: Optional[np.ndarray] = None
    phys_emiss: Optional[np.ndarray] = None
    phys_ndust: Optional[np.ndarray] = None
    phys_Z: Optional[np.ndarray] = None

    if args.compute_physics and len(data["nH"]) > 0:
        phys_xHI = cie_neutral_fraction(data["T"])
        phys_ne = electron_density(data["nH"], phys_xHI)
        phys_emiss = caseB_lya_emissivity(data["nH"], data["T"], phys_xHI, phys_ne)
        if args.metallicity is not None:
            phys_Z = np.full_like(data["nH"], args.metallicity)
            phys_ndust = laursen09_ndust(data["nH"], phys_xHI, phys_Z)
        print("  Physics computed: xHI, n_e, emissivity"
              + (", ndust, metallicity" if phys_ndust is not None else ""))

    # Keyword dict for optional physics columns
    physics_kw = dict(
        xHI=phys_xHI, n_e=phys_ne, emissivity=phys_emiss,
        ndust=phys_ndust, metallicity=phys_Z,
    )
    ...
    if output_is_hdf5(output):
        ...
        write_generic_text(
            output, x_out, y_out, z_out, data["level"], data["nH"], data["T"],
            data["vx"], data["vy"], data["vz"], boxlen_out, **physics_kw,
        )
