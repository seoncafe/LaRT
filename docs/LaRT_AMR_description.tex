...
are selected through Fortran procedure pointers set in \texttt{setup\_v2c.f90};
the outer simulation loop (\texttt{run\_simulation\_mod\_v2a}) and photon
injection (\texttt{gen\_photon\_car\_v2b}) are therefore unchanged.
...
\texttt{scatter\_resonance\_nostokes\_amr} implements the standard
Lyman-$\alpha$ resonance scattering algorithm (no Stokes polarisation):

\begin{enumerate}
  \item Sample the thermal velocity component of the scattering atom along the
        photon direction:
        \begin{equation}
          u_z = x_\nu + u_\ell \quad \text{(in the cell frame)}.
        \end{equation}
        The atom thermal velocity perpendicular to the photon direction is
        sampled from a Gaussian.
  \item Draw a new photon direction isotropically in the atom frame.
  \item Compute the new photon frequency in the atom frame, then transform back
        to the cell comoving frame.
  \item Apply the core-skip acceleration if $|x_\nu| < x_\mathrm{crit}$.
\end{enumerate}
