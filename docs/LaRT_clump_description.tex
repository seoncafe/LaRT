...
The test case \texttt{examples/clump\_sphere/clump\_test.in} sets
$R=1$, $r_c=0.01$, $N_\mathrm{cov}=5$, $\tau_0=1000$, $T=10^4\,\mathrm{K}$,
$10^5$ photons, 10 MPI ranks.  Typical results:
\begin{itemize}
  ...
  \item Wall time $\approx 3.75$ min (10 ranks).
\end{itemize}

The escaped spectrum $J_\mathrm{out}(x)$ shows the characteristic double-peak
expected for a clumpy Lyman-$\alpha$ source.  The observer peel-off
(\texttt{\_obs.fits.gz}) contains both \texttt{Scattered} (HDU~0) and
