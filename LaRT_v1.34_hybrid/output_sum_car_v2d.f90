...
contains
  ...
  if (par%save_all_photons) then
     call reduce_mem(allph%rp)
     call reduce_mem(allph%xfreq1)
     call reduce_mem(allph%xfreq2)
     call reduce_mem(allph%nscatt_HI)
     call reduce_mem(allph%nscatt_dust)
     if (trim(par%source_geometry) /= 'point') then
        call reduce_mem(allph%rp0)
     endif
     if (par%use_stokes) then
        call reduce_mem(allph%I)
        call reduce_mem(allph%Q)
        call reduce_mem(allph%U)
        call reduce_mem(allph%V)
     endif
  endif
