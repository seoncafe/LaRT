...
! local variables
  ...
  !endif
  !--- 2023.03.23
  if (mpar%nproc < 2) par%use_master_slave = .false.
  if (par%num_send_at_once * mpar%nproc > par%nphotons) then
     par%use_master_slave = .false.
  endif

  if (par%nx == 1 .or. par%ny == 1 .or. par%nz == 1) par%xyz_symmetry = .false.

  !--- Check if the observer is located inside the medium or not (2023.01.16).
  if (par%nside > 0) then
  ...
  !--- minor bug-fixed, 2023.03.23
  if (len_trim(par%dens_file) > 0 .and. get_extension(par%dens_file) == 'fits') then
  ...
  if (mpar%p_rank == 0) then
     ...
     write(*,'(2a)')       'Line ID                   : ', trim(par%line_id)
  ...
  real(kind=wp) :: wavelength,cext,albedo,hgg
  ...
  if (len_trim(scatt_mat_file) > 0) then
     if (mpar%h_rank == 0) then
        ...
        read(unit,*) wavelength,cext,albedo,hgg,scatt_mat%nPDF
        !read(unit,*) wavelength,albedo,hgg,scatt_mat%nPDF
        par%albedo    = albedo
        par%hgg       = hgg
        par%cext_dust = cext
        ! line%wavelength0 and wavelength is in units of micron.
        !line%wavelength0 = wavelength
        read(unit,*) string_tmp
     endif
     ...
     !call MPI_BCAST(line%wavelength0,   1, MPI_DOUBLE_PRECISION, 0, mpar%hostcomm, ierr)
  ...
  if (len_trim(line_prof_file) > 0) then
     if (mpar%h_rank == 0) then
        ...
        !--- speedc is given in km and wavelength is given in units of micron.
        if (par%line_prof_file_type == 0) then
           xfreq(:) = (xfreq(:) - speedc/(line%wavelength0*um2km))/Dfreq_ref
        else if (par%line_prof_file_type == 1) then
           xfreq(:) = -(xfreq(:) - line%wavelength0*1e4)/(line%wavelength0*1e4)*(speedc/vtherm_ref)
        endif
  ...
  !--- procedure pointer to calculate line profile
  !--- two voigt functions for line_type = 2
  select case(line%line_type)
  case (1)
     calc_voigt      => calc_voigt1
     do_resonance    => do_resonance1
  case (2)
     calc_voigt      => calc_voigt2
     do_resonance    => do_resonance2
  case (4)
     calc_voigt      => calc_voigt1
     do_resonance    => do_resonance4
  case (5)
     calc_voigt      => calc_voigt3
     do_resonance    => do_resonance5
  case (6)
     calc_voigt      => calc_voigt3
  ...
  if (par%use_clump_medium) then
     ...
     !--- Peel-off: clump-specific routines convert clump rest-frame frequency
     !    to the lab frame using each clump's bulk velocity (cl_vx/vy/vz).
     peeling_direct             => peeling_direct_clump
     peeling_dust_nostokes      => peeling_dust_nostokes_clump
     peeling_dust_stokes        => peeling_dust_stokes_clump
     peeling_resonance_nostokes => peeling_resonance_nostokes_clump
     peeling_resonance_stokes   => peeling_resonance_stokes_clump
  ...
  if (par%xyz_symmetry) then
     ...
     raytrace_to_edge => raytrace_to_edge_car_xyzsym
  else if (par%xy_symmetry) then
     raytrace_to_tau  => raytrace_to_tau_car_xysym
     raytrace_to_edge => raytrace_to_edge_car_xysym
     ...
     if (par%nx == 1 .and. par%ny == 1) then
        if (trim(par%geometry) == 'plane_atmosphere') then
           ...
           raytrace_to_edge => raytrace_to_edge_car_zonly
        endif
     else
        if (par%Omega /= 0.0_wp) then
           ...
           raytrace_to_edge => raytrace_to_edge_car_xyper
        endif
     endif
  endif
  ...
  if (par%observer_located_inside) then
     ...
     !peeling_dust_stokes        => peeling_dust_stokes_inside
     peeling_dust_nostokes      => peeling_dust_nostokes_inside
     !peeling_resonance_stokes   => peeling_resonance_stokes_inside
     peeling_resonance_nostokes => peeling_resonance_nostokes_inside
     observer_create            => observer_create_inside
     observer_destroy           => observer_destroy_inside
     output_reduce              => output_reduce_inside
     output_normalize           => output_normalize_inside
     write_output               => write_output_inside
     make_sightline_tau         => make_sightline_tau_inside
  else
     if (trim(par%source_geometry) == 'stellar_illumination') then
        peeling_direct          => peeling_direct_stellar_illumination
     else if (trim(par%source_geometry) == 'point_illumination') then
        peeling_direct          => peeling_direct_point_illumination
     else
